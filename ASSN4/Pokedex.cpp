...
{
	int length = pokedex.size();
	std::vector<std::string> temporary_name;
	...
	while (temporary_name.size() != 0) {
		...
		auto iter = temporary_name.begin();
		iter += (min_index);
		temporary_name.erase(iter);
...
void Pokedex::printBySearch(const std::string& name)
{
	...
	// print list
	std::vector<std::string> name_print_list;
	std::vector<int> number_print_list;
